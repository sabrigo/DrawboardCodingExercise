...
sequenceDiagram
    autonumber
    participant User
    participant View as XAML View (Page)
    participant VM as ViewModel
    participant Nav as INavigationService
    participant DI as Autofac / NavigationModule
    participant API as IAPIClient
    participant EA as IEventAggregator
    participant Shell as ShellViewModel
    participant UI as ThreadDispatcher / UI Thread
    participant UIInt as IUserInteractionService

    Note over App,DI: Application startup
    App->>DI: BuildContainer() (RegisterAssemblyModules)
    DI-->>App: IContainer
    App->>DI: Resolve Shell + ShellViewModel
    App->>Shell: Set Window.Current.Content
    Shell->>Shell: ShellViewModel.OnNavigatedToAsync(null)
    Shell->>Nav: NavigateAsync(PageKey.Welcome)
    Nav->>DI: Resolve WelcomeView + WelcomeViewModel
    DI-->>Nav: instances
    Nav->>UI: Dispatch navigation -> Frame.Navigate(WelcomeView)
    Nav->>WelcomeViewModel: If INavigateToAware -> OnNavigatedToAsync(parameter)

    Note over User,View: User triggers list load
    User->>View: Open Page / Click Refresh
    View->>VM: Execute LoadCommand / OnNavigatedToAsync
    VM->>EA: Post NotifyBusyEvent("LoadList")
    EA->>Shell: NotifyBusyEvent -> Shell shows busy indicator
    VM->>API: GetAsync("/departments" or "/films")
    API-->>VM: JSON list (or throws)
    alt success
        VM->>UI: Dispatch collection updates (ThreadDispatcher)
        UI-->>View: Items bound & displayed
        VM->>EA: Post NotifyDoneEvent("LoadList")
        EA->>Shell: NotifyDoneEvent -> Shell clears busy
    else failure
        VM->>UIInt: ShowRetryDialogAsync()
        UIInt-->>VM: Retry or Cancel
        alt Retry
            VM->>API: GetAsync(...)  -- retry
        else Cancel
            VM->>EA: Post NotifyDoneEvent("LoadList")
        end
    end

    Note over User,View: User selects item -> navigate to detail
    View->>VM: ItemSelectedCommand(itemId)
    VM->>Nav: NavigateAsync(PageKey.Detail, itemId)
    Nav->>DI: Resolve DetailView + DetailViewModel
    DI-->>Nav: instances
    Nav->>UI: Frame.Navigate(DetailView)
    Nav->>DetailViewModel: OnNavigatedToAsync(itemId)
    DetailViewModel->>EA: Post NotifyBusyEvent("LoadDetail")
    DetailViewModel->>API: GetAsync("/objects/{id}" or "/films/{id}")
    API-->>DetailViewModel: JSON detail
    alt has image
        DetailViewModel->>API: GetImageAsync(imageUrl)
        API-->>DetailViewModel: Image stream
    end
    DetailViewModel->>UI: Dispatch property + image updates
    UI-->>View: Detail UI updates (title, episode, release, crawl, thumbnails...)
    DetailViewModel->>EA: Post NotifyDoneEvent("LoadDetail")
    EA->>Shell: Shell clears busy